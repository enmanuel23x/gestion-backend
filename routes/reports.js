...
router.get('/get_client', async (req, res) => {
    const request = await pool.query(`SELECT * FROM client`);
    //console.log(request);	    console.log(request);
    res.json(request);
});
// Ruta para listar requests en base a un cli_id
router.get('/get_reqbycli/:id', async (req, res) => {
    const {id} = req.params
    const ESTADO = 'open';
  const request = await pool.query(`SELECT * FROM request where sta_id = '${ESTADO}' AND cli_id = '${id}' ORDER BY req_title ASC`);
    //console.log(request);	    console.log(request);
    res.json(request);
});
